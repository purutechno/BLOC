...
import 'package:ioesolutions/src/screens/note_types.dart';
import 'package:ioesolutions/src/screens/syllabus_content.dart';

import '../helper_functions.dart';
class NoteSubjects extends StatelessWidget {
  final String content_type;
  final String faculty_code;
  final String semester_id;
  const NoteSubjects({Key key,this.content_type,this.faculty_code,this.semester_id}) : super(key: key);
...
class NoteSubjectsBody extends StatelessWidget {
  const NoteSubjectsBody({
    Key key,
  }) : super(key: key);
     ...
     child: 
      ...
      builder: (context, state) {
        if(state is ContentLoaded)
        {
          ...
          {
            return Center(
              child: Text("Notes for this semester will be available soon!",style: TextStyle(color: Colors.grey[600]),),
            ); 
          }
