...
class DownloadTestPage extends StatefulWidget with WidgetsBindingObserver {
  ...
  final notes;
  DownloadTestPage({Key key,this.subject_name,this.note_type,this.notes}) : super(key: key);
  ...
  @override
  _NoteFilesListState createState() => new _NoteFilesListState(subject_name: subject_name,note_type: note_type,notes: notes);
}

class _NoteFilesListState extends State<DownloadTestPage> {
  final String subject_name;
  final String note_type;
  final notes;
  var _documents = [];
  _NoteFilesListState({this.subject_name,this.note_type,this.notes});
  String _getFileName(filename)
  {
    if(filename.length>50)
    {
  ...
  _prepareSubjectsList()
  {
    ...
    {
      print("File id is : ${notes[i]["file_id"]}");
      _documents.add(
        {
