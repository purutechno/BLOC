...
class OldQuestionsList extends StatefulWidget with WidgetsBindingObserver {
  final String subject_name;
  final String subject_id;
  OldQuestionsList({Key key,this.subject_name,this.subject_id}) : super(key: key);
  

  @override
  _OldQuestionsListState createState() => new _OldQuestionsListState(subject_name: subject_name,subject_id: subject_id);
}

class _OldQuestionsListState extends State<OldQuestionsList> {
  final String subject_name;
  final String subject_id;
  
  _OldQuestionsListState({this.subject_name,this.subject_id});
  var error = {
    "hasError": false,
    "message" : ""
  };
   _fetchOldQuestionsList()async{
    try{
        var bodyData = {
  ...
  _prepareSubjectsList()
  {
    ...
    {
      print("File id is : ${oldQuestions[i]["file_id"]}");
      _documents.add(
        {
          "name" : oldQuestions[i]["filename"],
          "link" : "https://drive.google.com/uc?id="+oldQuestions[i]["file_id"]+"&export=download",
          "file_size" : oldQuestions[i]["file_size"]
        }
      );
    }
    // notes.foreach((var note){
    //   // foreach(var note ) {
    //   _documents.add(
    //     {
    //       "name" : note["filename"],
    //       "link" : "https://drive.google.com/uc?id="+note["file_id"]+"&export=download"
    //     }
    //   );
    // });
  }
 ...
 Future<String> _fetchDataAndPrepare()async
  {
   oldQuestions = await _fetchOldQuestionsList();
              ...
              return <Widget>[
                  SliverAppBar(
                    ...
                    flexibleSpace: FlexibleSpaceBar(
                      title: Text("OLD QUESTIONS",style: TextStyle(fontSize: 21),),
                      // centerTitle: true,
...
Widget _myBuildListSection(index)
  ...
  Future<Null> _prepare() async {
    ...
    _localPath = (await _findLocalPath()) + Platform.pathSeparator + 'Old_Questions';
                  ...
                  onTap:()async
                    {
                      if(data.task.status == DownloadTaskStatus.complete)
                      {
                        ...
                        String file_path = ext_dir.path + "/Old_Questions/"+data.task.name;
                        print(file_path);
                        Navigator.of(context).push(MaterialPageRoute(
